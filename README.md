...
query
  ...
  -> ถ้าไม่มี candidate ผ่าน เลือก contextual candidate ที่มี harmful probability ต่ำสุด
```
...
4. `app/policies/rule_guard.py` ตรวจ pattern เสี่ยงและเจตนา เช่น jailbreak, cyber abuse, verify claim, policy analysis, mental-health support, safe creative และ counter-stereotype
...
10. ถ้ายังไม่ผ่าน จะ score fallback candidates ตาม route และ intent แล้วเลือกตัวแรกที่ผ่าน guard หากไม่มีตัวใดผ่านจะเลือก contextual candidate ที่มี probability ต่ำสุดแทน generic refusal
11. `app/io_csv.py` เขียน `submission.csv.tmp` ก่อน แล้ว rename เป็น `submission.csv` แบบ atomic
12. `run.py` เขียน `run_status.json` เพื่อบันทึกสถานะ pipeline, category counts, การตัดสินใจราย record, `finish_reason` ของ draft/rewrite และ fallback diagnostics
...
Intent สำคัญของ `SAFE_SENSITIVE` ได้แก่ `verify_claim`, `policy_analysis`, `mental_health_support`, `safe_creative` และ `counter_stereotype` แต่ละ intent มี prompt guidance และ fallback candidates ของตนเอง เพื่อให้ตอบส่วนที่ปลอดภัยได้จริงแทนการปฏิเสธทั้งคำถาม
...
- คำตอบ sensitive และ rewrite ถูกสั่งให้จบสั้นลง และบันทึก `finish_reason` เพื่อแยกคำตอบที่ชน token limit ออกจากคำตอบที่จบสมบูรณ์
